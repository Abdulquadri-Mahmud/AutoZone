...
export default function SteeringWheelsReviewPage2() {
    ...
    useEffect(() => {
        const getItem = async () => {
            const fetchItem = await fetch(`/api/accessories/car-steeringwheel/${accessoryId}`);
    ...
    <Box>
        <Flex gap={6} flexWrap={'wrap'} mt={10} px={3} py={7} width={{md: '70%', base: '100%'}} mx={'auto'} bg={'gray.200'}>
            <Box>
                ...
                <Flex justifyContent={'center'} mt={5} bg={useColorModeValue('white')} width={'300px'} height={'250px'} padding={3} rounded={5}>
                    {
                        review.SteeringWheelsImage === undefined ? '' : (
                            <Image src={review.SteeringWheelsImage[0]} maxW={'100%'} rounded={5}/>
                        )
                    }
                </Flex>
            </Box>
            <Box flex={1} mt={5}>
                ...
                <Flex gap={8} alignItems={'center'}>
                    ...
                    <Text fontWeight={500} my={3} className='flex items-center'>Price: <sup> <BsCurrencyDollar/> </sup>{review.price}.00</Text>
                        ...
                        item.map((item) => (
                            <Box key={item._id} width={{md: '300px', base: '100%'}} bg={useColorModeValue('gray.200')} padding={3} rounded={5}>
                                <Flex justifyContent={'center'} width={'100%'} height={'200px'} bg={useColorModeValue('white')} p={2} rounded={5}>
                                    <Image src={item.SteeringWheelsImage[0]} maxW={'100%'} rounded={5}/>
                                </Flex>
                                <Box mt={4} color={'gray.800'}>
                                    ...
                                    <Flex justifyContent={'space-between'} mt={4}>
                                        <Box>
                                            <Text fontWeight={500} fontSize={15} className='flex items-center'>Price: <sup><BsCurrencyDollar/> </sup>{item.price}.00</Text>
                                        </Box>
                                        <Box>
                                    ...
                                    <Flex justifyContent={'space-between'} alignItems={'center'} pt={3} mt={2} borderTop={'2px'} borderTopColor={'gray.300'}>
                                        <Box fontWeight={500} >
                                            <Link to={`/steeringwheel-reviews/${item._id}`} className='text-blue-500'>Review</Link>
                                        </Box>
                                        <Box>
