...
export default function StereoReviewsPage2() {
    ...
    <Box>
        ...
        <Flex mt={10} flexWrap={'wrap'} gap={6} alignItems={''} px={3} py={7} width={{md: '90%', base: '97%'}} rounded={5} mx={'auto'} bg={useColorModeValue('gray.200', 'gray.700')}>
            ...
            <Box mt={5} flex={1}>
                ...
                <Text fontWeight={500} mt={3} fontSize={{md:16, base:14}} wordBreak={'break-word'}>{review.descriptions}</Text>
                <Flex gap={4} mt={2}>
                ...
                </Flex>
                    {
                        review.specialFeatures && (
                            <Flex gap={2} mt={3} alignItems={'center'}>
                                <Text fontWeight={500} color={useColorModeValue('black', 'gray.200')}>Special Features: </Text>
                                <Text fontWeight={500} color={useColorModeValue('black', 'gray.400')} fontSize={13}>{review.specialFeatures}</Text>
                            </Flex>
                        )
                    }
                    {
                        review.screenSize && (
                            <Text fontWeight={500} mt={4}  color={useColorModeValue('black', 'gray.200')} fontSize={17}>Screen Size: <span className='text-sm font-medium'>{review.screenSize}</span></Text>
                        )
                    }
                    {
                ...
                <Box mt={3} py={1} px={2} bg={'white'} width={'200px'} rounded={4}>
                    <Flex gap={2} alignItems={'center'}>
                        ...
                        <Flex alignItems={'center'} fontWeight={500} color={'red.500'} fontSize={16}>
                            <BsCurrencyDollar/> {review.price}
                        </Flex>
                    </Flex>
                    {
                        review.prevprice && (
        ...
        <Box maxW={'100%'} mx={'auto'} mt={6}>
            ...
            <Flex mt={20} gap={3} justifyContent={'center'} flexWrap={'wrap'}>
                ...
                items.length > 0 ? (
                    items.map((item) => (
                    item.category === 'Steroes' ? (
                        <Box width={{md: '350px', base: '350px'}} bg={useColorModeValue('gray.200', 'gray.700')} padding={3} rounded={5}>
                            ...
                            <Box mt={4} color={useColorModeValue('gray.800', 'gray.200')}>
                                ...
                                <Flex justifyContent={'center'} alignItems={'center'} pt={3} mt={2} borderTop={'1px'} borderTopColor={'gray.600'}>
                                    <Flex justifyContent={'center'} alignItems={'center'} fontWeight={500} bg={useColorModeValue('blue.500', 'white')} color={useColorModeValue('white', 'black')} width={'100%'} height={'40px'} rounded={3}>
                                        <Link to={`/steroes-reviews/${item._id}`} className=''>Review</Link>
                                    </Flex>
                                </Flex>
                            </Box>
                        </Box>
