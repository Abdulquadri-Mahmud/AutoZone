...
import HedlightReview from '../../../components/Accessories/Reviews/HedlightReview';

export const HedlightReviewContext = createContext();

export default function HedlightReviewPage() {
    ...
    useEffect(() => {
        const getISingletem = async () => {
          const fetchItem = await fetch(`/api/accessories/car-headlight/${accessoryId}`);
  ...
  return (
    <Box>
        <HedlightReviewContext.Provider value={review}>
            <HedlightReview review={review} key={review._id}/>
        </HedlightReviewContext.Provider>
                    ...
                    item.map((item) => (
                        <Box key={item._id} width={{md: '300px', base: '100%'}} bg={useColorModeValue('gray.200')} padding={3} rounded={5}>
                            <Flex justifyContent={'center'} width={'100%'} height={'200px'} bg={useColorModeValue('white')} p={2} rounded={5}>
                                <Image src={item.HeadLightImage[0]} maxW={'100%'} rounded={5}/>
                            </Flex>
                            <Box mt={4} color={'gray.800'}>
                                ...
                                <Flex justifyContent={'space-between'} alignItems={'center'} pt={3} mt={2} borderTop={'2px'} borderTopColor={'gray.300'}>
                                    <Box fontWeight={500} >
                                        <Link to={`/review-headlight-reviews/${item._id}`} className='text-blue-500'>Review</Link>
                                    </Box>
                                    <Box>
