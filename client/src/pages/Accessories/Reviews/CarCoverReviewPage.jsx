...
import CarCoverReview from '../../../components/Accessories/Reviews/CarCoverReview';

export const CarCoverReviewContext = createContext();

export default function CarCoverReviewPage() {
    ...
    useEffect(() => {
        const getItemId = async () => {
            try {
                const fetchItem = await fetch(`/api/accessories/car-car-cover/${accessoryId}`);
  ...
  return (
    <Box>
        <CarCoverReviewContext.Provider value={review}>
            <CarCoverReview review={review}/>
        </CarCoverReviewContext.Provider>
                            ...
                            <Box key={item._id} width={{md: '300px', base: '100%'}} bg={useColorModeValue('gray.200')} padding={3} rounded={5}>
                                ...
                                <Box mt={4} color={'gray.800'}>
                                    ...
                                    <Flex justifyContent={'space-between'} alignItems={'center'} pt={3} mt={2} borderTop={'2px'} borderTopColor={'gray.300'}>
                                        <Box fontWeight={500} >
                                            <Link to={`/review-car-cover-reviews/${item._id}`} className='text-blue-500'>Review</Link>
                                        </Box>
                                        <Box>
