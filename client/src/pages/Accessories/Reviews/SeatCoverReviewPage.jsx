...
import SeatCoverReview from '../../../components/Accessories/Reviews/SeatCoverReview';

export const SeatCoverReviewContext = createContext();

export default function SeatCoverReviewPage() {
    ...
    useEffect(() => {
        const getItemId = async () => {
            try {
                const fetchItem = await fetch(`/api/accessories/car-seat-cover/${accessoryId}`);
  ...
  return (
    <Box>
        <SeatCoverReviewContext.Provider value={review}>
            <SeatCoverReview review={review}/>
        </SeatCoverReviewContext.Provider>
                        ...
                        item.map((item) => (
                            <Box key={item._id} width={{md: '300px', base: '100%'}} bg={useColorModeValue('gray.200')} padding={3} rounded={5}>
                                <Flex justifyContent={'center'} width={'100%'} height={'200px'} bg={useColorModeValue('white')} p={2} rounded={5}>
                                    <Image src={item.SeatCoverImage[0]} maxW={'100%'} rounded={5}/>
                                </Flex>
                                <Box mt={4} color={'gray.800'}>
                                    ...
                                    <Flex justifyContent={'space-between'} alignItems={'center'} pt={3} mt={2} borderTop={'2px'} borderTopColor={'gray.300'}>
                                        <Box fontWeight={500} >
                                            <Link to={`/review-seat-cover-reviews/${item._id}`} className='text-blue-500'>Review</Link>
                                        </Box>
                                        <Box>
