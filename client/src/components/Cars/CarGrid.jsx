...
export default function CarGrid() {
  ...
  //  let emptyCart = []
   ...
   const handleAddToCart = (car) => {
    ...
    // localStorage.setItem('grid-cart', JSON.stringify(emptyCart));
  ...
  return (
    <>
      <Header/>
      <Flex justifyContent={'center'} flexWrap={'wrap'} gap={4} py={'2rem'} mx={{md: 0, base: 2}}>
        {
          cars.length > 0 ? (
            cars.map((car) => (
              <Box key={car._id} width={{md: '350px', base: '100%'}} padding={3} bg={useColorModeValue('white', 'gray.700')} shadow={'md'} borderRadius={5} position={'relative'} className='font-medium'>
                <Flex justifyContent={'center'} alignItems={'center'} width={'100%'} mt={4} position={'relative'}>
                  <Box width={'100%'} height={'250px'} bg={useColorModeValue('gray.200')}>
                    <Image src={car.carimage[0]} alt={car.name} maxW={'100%'} maxH={'250px'} objectFit={'contain'}></Image>
                  </Box>
                  <Box position={'absolute'} bottom={0} bg={useColorModeValue('white', 'gray.800')} px={2} py={2} rounded={4}>
                ...
                <Flex justifyContent={'space-between'} alignItems={'center'}>
                  <Box bg={useColorModeValue('blue.500', 'gray.800')} px={3} py={1} color={'white'}  position={'absolute'} top={2} rounded={3} left={2}>
                ...
                <Flex justifyContent={'space-between'} mt={2} color={useColorModeValue('gray.600', 'gray.200')}>
