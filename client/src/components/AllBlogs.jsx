        ...
        <Box p={{md: '2rem', base: '0.5rem'}} py={{md: 10, base: 10}} mt={14} maxW={{md: '95%', base: '100%'}} mx={'auto'} rounded={10} bg={useColorModeValue('gray.200', 'gray.800')} shadow={'md'}>
            <Box>
                <Box width={{md: '300px', base: '70%'}} bg={useColorModeValue('blue.500', 'gray.700')} p={4} borderBottomWidth={1} borderBottomColor={'gray.300'}>
                    <Heading fontWeight={500} fontSize={22} color={'white'}>LATEST BLOG POST</Heading>
                </Box>
                            ...
                            blogs.map((blog) => (
                                <Box className="blog" key={blog._id} width={{base: '100%', md:'32%'}} h={{base: '100%', md:'100%'}} rounded={10} position={'relative'} bg={useColorModeValue('white', 'gray.700')} color={'black'}>
                                    ...
                                    <Box roundedBottom={10}padding={3} color={useColorModeValue('black', 'white')}>
                                        <Text fontWeight={500} py={1} color={'blue.500'} fontSize={16}>{blog.title}</Text>
                                        {/* <Text fontWeight={500} textDecor={'underline'} color={'red.500'} >{blog.snippet}</Text>  */}
                                        <Text fontWeight={500} py={1}>{blog.body.slice(0,150)} ...</Text>
                                        <Button height={'40px'} mt={4} bg={useColorModeValue('blue.500', 'gray.200')} color={useColorModeValue('white', 'black')} _hover={{opacity: 0.7}}>
