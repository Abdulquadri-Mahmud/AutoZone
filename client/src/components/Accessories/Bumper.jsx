...
import { BumpersContext } from '../../pages/Accessories/BumperPage';

export default function Bumper() {
    const bumpers = useContext(BumpersContext);
        ...
        <Flex gap={3} justifyContent={'center'} flexWrap={'wrap'}>
            {
                    ...
                    bumpers.map((bumper) => (
                        <Box key={bumper._id} width={{md: '300px', base: '100%'}} bg={useColorModeValue('gray.200')} padding={3} rounded={5}>
                            ...
                            <Box mt={4} color={'gray.800'}>
                                <Heading mb={2} fontWeight={500} fontSize={16} color={'blue.500'} isTruncated>{bumper.name}</Heading>
                                <Box>
                                    <Text fontWeight={500}>{bumper.descriptions.slice(0, 100)}...</Text>
                                </Box>
                                <Flex justifyContent={'space-between'} mt={4}>
                                    <Box>
                                        <Text fontWeight={500} fontSize={15} className='flex items-center'>Price: <BsCurrencyDollar/>{bumper.price}</Text>
                                    </Box>
                                    <Box>
                                        <Text fontWeight={500} fontSize={15}>Deal: {bumper.deal}</Text>
                                    </Box>
                                </Flex>
                ...
                :(
                    <Text>No Bumpers accessories to Display!</Text>
                )
            }
        </Flex>
    </Box>
