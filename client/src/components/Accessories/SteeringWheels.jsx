...
import { SteeringWheelsContext } from '../../pages/Accessories/SteeringWheelsPage';


export default function SteeringWheels() {
    const steeringwheels = useContext(SteeringWheelsContext);

  return (
    <Box py={'3rem'}>
        <Flex gap={3} justifyContent={'center'} flexWrap={'wrap'}>
            {
                steeringwheels.length > 0 ? (
                    steeringwheels.map((steeringwheel) => (
                        <Box key={steeringwheel._id} width={{md: '300px', base: '100%'}} bg={useColorModeValue('gray.200')} padding={3} rounded={5}>
                            ...
                            <Box mt={4} color={'gray.800'}>
                                ...
                                <Flex justifyContent={'space-between'} mt={4}>
                                    <Box>
                                        <Text fontWeight={500} fontSize={15} className='flex items-center'>Price: <BsCurrencyDollar/>{steeringwheel.price}</Text>
                                    </Box>
                                    <Box>
                                        <Text fontWeight={500} fontSize={15}>Deal: {steeringwheel.deal}</Text>
                                    </Box>
                                </Flex>
                                <Flex justifyContent={'space-between'} alignItems={'center'} pt={3} mt={2} borderTop={'2px'} borderTopColor={'gray.300'}>
                                    <Box fontWeight={500} >
                                        <Link to={`/steeringwheel-reviews/${steeringwheel._id}`} className='text-blue-500'>Review</Link>
                                    </Box>
                                    <Box>
