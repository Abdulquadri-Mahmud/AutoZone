...
import { SunshadesReviewContext } from '../../../pages/Accessories/Reviews/SunshadesReviewPage';
import { IoStar } from 'react-icons/io5';

export default function SunshadesReview() {
    const item = useContext(SunshadesReviewContext);
            ...
            <Flex justifyContent={{md: 'start', base: 'center'}}>
                {
                    item.SunShadesImage === undefined ? '' : (
                        ...
                        position={'relative'} height={{md:'350px', base: '300px'}} width={{md:'350px', base: '300px'}} p={3} rounded={5}>
                            <Image src={item.SunShadesImage[0]} ref={display} maxW={'100%'} rounded={5}/>
                            <Flex justifyContent={'center'} position={'absolute'} top={0} left={0} width={'70px'} borderTopLeftRadius={5} py={1} bg={'gray.200'}>
                                <Text fontWeight={500}>{item.SunShadesImage.length} Photo</Text>
                            </Flex>
                        </Flex>
                    )
                }
            ...
            <Flex alignItems={'center'} justifyContent={'center'} bg={useColorModeValue('')} gap={3} flexWrap={'wrap'}  p={1}>
                {
                    item.SunShadesImage === undefined ? '' : (
                        <>
                            {
                                item.SunShadesImage.map((img, index) => (
