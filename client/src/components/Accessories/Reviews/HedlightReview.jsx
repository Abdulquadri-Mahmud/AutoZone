...
import { HedlightReviewContext } from '../../../pages/Accessories/Reviews/HedlightReviewPage';

export default function HedlightReview() {
    const item = useContext(HedlightReviewContext);
    console.log(item);
    ...
    <Flex mt={10} flexWrap={'wrap'} gap={6} px={3} py={7} width={{md: '90%', base: '100%'}} mx={'auto'} bg={'gray.100'}>
            ...
            <Flex justifyContent={{md: 'start', base: 'center'}}>
                {
                    item.HeadLightImage === undefined ? '' : (
                        ...
                        position={'relative'} height={{md:'350px', base: '300px'}} width={{md:'350px', base: '300px'}} p={3} rounded={5}>
                            <Image src={item.HeadLightImage[0]} ref={display} maxW={'100%'} rounded={5}/>
                            <Flex justifyContent={'center'} position={'absolute'} top={0} left={0} width={'70px'} borderTopLeftRadius={5} py={1} bg={'gray.200'}>
                                <Text fontWeight={500}>{item.HeadLightImage.length} Photo</Text>
                            </Flex>
                        </Flex>
                    )
                }
            ...
            <Flex alignItems={'center'} justifyContent={'center'} bg={useColorModeValue('')} gap={3} flexWrap={'wrap'}  p={1}>
                {
                    item.HeadLightImage === undefined ? '' : (
                        <>
                            {
                                item.HeadLightImage.map((img, index) => (
        ...
        <Box mt={5} flex={1}>
            ...
            <Flex gap={4} mt={4}>
                ...
                <Text className='font-medium'>Year: <span className='font-normal'>{item.model}</span></Text>
            </Flex>
