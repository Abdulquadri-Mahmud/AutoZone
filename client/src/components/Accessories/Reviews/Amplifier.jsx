...
export default function Amplifier() {
    ...
    return (
        <Flex mt={10} flexWrap={'wrap'} gap={6} px={3} py={7} width={{md: '90%', base: '100%'}} mx={'auto'} bg={useColorModeValue('gray.200', 'gray.700')}>
                ...
                <Flex justifyContent={{md: 'center', base: 'center'}} >
                    {
                        item.accessoryImage === undefined ? '' : (
                            <Flex justifyContent={'center'} alignItems={'center'} position={'relative'} height={{md:'350px', base: '350px'}} width={{md:'350px', base: '300px'}} p={3} rounded={5}>
                                <Image src={item.accessoryImage[0]} ref={display} maxW={'300px'} maxH={'300px'} rounded={5}/>
            ...
            <Box mt={5} flex={1}>
                ...
                <Flex gap={4} mt={2}>
                    <Text className='font-medium' color={'blue.500'} fontSize={17}>Make: <span className='font-medium text-white text-sm'>{item.make}</span></Text> |
                    <Text className='font-medium' color={'blue.500'} fontSize={17}>Year: <span className='font-medium text-white text-sm'>{item.year}</span></Text>
                </Flex>
                {
