...
import { DoorsReviewContext } from '../../../pages/Accessories/Reviews/DoorReviewsPage';
import { IoStar } from 'react-icons/io5';


export default function DoorReviews() {
    const item = useContext(DoorsReviewContext);
            ...
            <Flex justifyContent={{md: 'start', base: 'center'}}>
                {
                    item.DoorImage === undefined ? '' : (
                        ...
                        position={'relative'} height={{md:'350px', base: '300px'}} width={{md:'350px', base: '300px'}} p={3} rounded={5}>
                            <Image src={item.DoorImage[0]} ref={display} maxW={'100%'} rounded={5}/>
                            <Flex justifyContent={'center'} position={'absolute'} top={0} left={0} width={'70px'} borderTopLeftRadius={5} py={1} bg={'gray.200'}>
                                <Text fontWeight={500}>{item.DoorImage.length} Photo</Text>
                            </Flex>
                        </Flex>
                    )
                }
